...
$.fn.JPaging= function(param){
  ...
  var params=$.extend({pageSize:10,visiblePageSize:5},param);
    ...
    var $countLi = $("ul#listPage li").length;
    var $currentIndex = 2;
    ...
    if ($pageCount >= 1 && $visiblePageSize >= 1) {
        $(this).append("<a href='javascript:void(0)' style='font-weight:700;'>" + "<");
        $(this).append("<a href='javascript:void(0)' id='pre_point' class='hidden'>" + "...");
        for (var i = 1; i <= $pageCount; i++) {
            if (i <= $visiblePageSize) {
                $(this).append("<a href='javascript:void(0)'>" + i + "</a>");
            }
            else if (i > $visiblePageSize) {
                $(this).append("<a href='javascript:void(0)' class='hidden'>" + i + "</a>");
            }
        }
        $(this).append("<a href='javascript:void(0)' id='next_point'>" + "...");
        $(this).append("<a href='javascript:void(0)' style='font-weight:700;'>" + ">");
        $("ul#listPage li:gt(" + ($pageSize - 1) + ")").hide();
    ...
    $("#pre_point").on('click', function(event) {
        ...
        $("#paging a:eq(" + (prevIndex - 1) + ")").removeClass('hidden').addClass('aktif');
        $currentIndex = prevIndex - 1;
        var gt = $pageSize * ($currentIndex - 1);
        $("#listPage li").hide();
        for (var i = gt - $pageSize; i < gt; i++) {
            $("ul#listPage li:eq(" + i + ")").show();
        }
        if (($currentIndex - 1) == $pageCount && $visiblePageSize < $pageCount) {
            $("#next_point").addClass('hidden');
        }
        else if ($currentIndex < ($pageCount + $visiblePageSize) && $visiblePageSize < $pageCount) {
    ...
    $(this).on("click", "a:not('#pre_point,#next_point')", function() {
        var $index = $(this).index();
        console.log("curindex:" + $currentIndex + " visible_page_count:" + $visiblePageSize + " pageCount:" + $pageCount);
        if ($(this).is("#paging a:first") === true) {
            ...
            if ($("#paging a.hidden").length >= 1) {
                $("#paging a:not('#next_point,#paging a:last'):eq(" + ($currentIndex + $visiblePageSize) + ")").addClass("hidden");
            }
            $("ul#listPage li").hide();
            for (var f = gtFirst - $pageSize; f < gtFirst; f++) {
                $("ul#listPage li:eq(" + f + ")").show();
            }
            console.log("end curindex:" + $currentIndex + " visible_page_count:" + $visiblePageSize + " pageCount:" + $pageCount);
            return false;
        }
